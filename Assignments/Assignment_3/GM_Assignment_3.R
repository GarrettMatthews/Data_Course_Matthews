...
list.files(recursive = TRUE, pattern = "thatch_ant.csv") -> path
...
as.numeric(dat$Headwidth) -> dat$Headwidth
head(dat$Headwidth)

plot(dat$Mass,dat$Headwidth)
...
#fill it in
size_class_mean_mass = data.frame(Levels = levels(dat$Size.class),
                                  Mean = new_vector)
data.frame("Size Class" = levels(dat$Size.class), "Mean Mass" = new_vector) -> size_class_mean_mass
...
summary(dat$Colony)
factor(dat$Colony) -> dat$Colony
plot(dat$Headwidth,dat$Mass, main = "Headwidth vs Mass", xlab = "Headwidth", ylab = "Mass", col=dat$Colony, cex = 1.5)
legend('topleft', legend = levels(dat$Colony), col = c(rainbow(11)), title = 'Ant Colonies', horiz = TRUE, cex = 0.65, pch = 1)
?plot()
?legend()
palette()
# 2.  Write the code to save it (with meaningful labels) as a jpeg file
?jpeg()
jpeg(filename = "./GM_Headwidth_vs_Mass.jpg")
plot(dat$Headwidth,dat$Mass, main = "Headwidth vs Mass", xlab = "Headwidth", ylab = "Mass", col=dat$Colony, cex = 1.5)
legend('topleft', legend = levels(dat$Colony), col = c(rainbow(11)), title = 'Ant Colonies', horiz = TRUE, cex = 0.65, pch = 1)
dev.off()
...
as.numeric(dat$Colony) -> dat$Colony
dat[dat$Colony == c(1,2), ] -> dat_1

# 4.  Write code to save this new subset as a .csv file
?write.csv()
write.csv(dat_1, file = "./ant_col_1-2.csv", quote = FALSE)
